...
public interface NhanVienRepository extends JpaRepository<NhanVien, Integer> {
    ...
    @Query(value = "SELECT * FROM NhanVien WHERE " +
            ...
            "(:status IS NULL OR :status = '' OR CAST(TrangThai AS VARCHAR(5)) = :status)", nativeQuery = true)
    List<NhanVien> filter(@Param("keyword") String keyword,
